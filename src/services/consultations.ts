 ...
 * 사용자의 상담 기록 조회 (최신순)
 */
async function getByUser(limit = 50): Promise<Consultation[]> {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('로그인이 필요합니다');
 ...
 * 특정 날짜 범위의 상담 기록 조회
 */
async function getByDateRange(startDate: string, endDate: string): Promise<Consultation[]> {
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error('로그인이 필요합니다');
